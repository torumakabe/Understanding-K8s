...
resource "azuread_application" "aks" {
  ...
  identifier_uris = ["https://${var.prefix}-k8sbook-${var.chap}-sp-aks-blue-${var.cluster_type}"]
  ...
  provisioner "local-exec" {
    ...
    while :
    do
        OID=$(az ad app show --id "https://${var.prefix}-k8sbook-${var.chap}-sp-aks-blue-${var.cluster_type}" -o tsv --query objectId)
        if [ -n "$OID" ]; then
...
resource "azuread_service_principal" "aks" {
  ...
  provisioner "local-exec" {
    ...
    while :
    do
        SP_OID=$(az ad sp show --id "https://${var.prefix}-k8sbook-${var.chap}-sp-aks-blue-${var.cluster_type}" -o tsv --query objectId)
        if [ -n "$SP_OID" ]; then
